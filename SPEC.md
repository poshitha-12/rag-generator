...
- **NFR5** — New dependencies must not introduce a PyTorch/CUDA
  ...
  include torch, but it has torch-pulling optional extras
  (`sentence-transformers`, `transformers`) — install plain `ragas`
  with no extras, and verify after install (`pip list | grep -iE
  "torch|tensorflow"` returns nothing) before treating this as done.

## Definition of Done

- [x] All of FR1-FR9 manually verified with two unrelated document sets
- [x] `pytest` passes
...
- [x] README run instructions work verbatim on a machine that isn't yours
- [x] `eval/RESULTS.md` exists, generated by `python eval/run_eval.py`
      against the real configured generation model, with per-question
      RAGAS scores (faithfulness, answer relevancy, and — where a
      reference answer exists — context precision/recall and answer
      correctness)
...
| NFR4 | `eval/run_eval.py` — not part of `make verify` (needs a real API key, real tokens, and now the `ragas` dependency); run manually, see `eval/RESULTS.md` |
